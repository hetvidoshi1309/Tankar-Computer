...
class Carousel extends Component {
	...
	changeSlide(slide) {
		...
		if (slide >= 0 && slide <= React.Children.count(this.props.children) + 1) this.setState({ slide, sliding: true, dragging: null, offset: 0 }, this.setTimer);
	}
		...
		return React.createElement(
			...
			enabled && indicator && React.createElement(
				...
				Children.map(children, (item, index) => React.createElement(
					'li',
					{ 'aria-current': slide === index + 1, onClick: this.changeSlide.bind(this, index + 1) },
					index + 1
				))
			),
			enabled && switcher && React.createElement(
				'div',
				null,
				React.createElement('button', { type: 'button', className: 'prev', onClick: goPrevSlide }),
				React.createElement('button', { type: 'button', className: 'next', onClick: goNextSlide })
			)
		);
	}
}
